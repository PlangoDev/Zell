/* ─────────────────────────────────────────────────────────────────────────────
 * common.h — shared host helpers (carried from test_012): xorshift64* RNG, a
 * monotonic timer, stable softmax/argmax, and the RESTRICT alias hint. All
 * header-only (static inline) so it drops into both .c and .cu translation units.
 * ───────────────────────────────────────────────────────────────────────────*/
#ifndef BF_COMMON_H
#define BF_COMMON_H
...
/* ── xorshift64*: one 64-bit word of non-zero state, a few instructions/number ── */
...
static inline double rng_unif(rng_t *r) {            /* [0,1) */
    return (rng_u64(r) >> 11) * (1.0 / 9007199254740992.0);
}
static inline int rng_int(rng_t *r, int n) {         /* [0,n) */
    return (int)(rng_u64(r) % (uint64_t)n);
}
static inline double rng_gauss(rng_t *r) {           /* standard normal (Box-Muller) */
...
/* ── stable in-place softmax over n logits ── */
